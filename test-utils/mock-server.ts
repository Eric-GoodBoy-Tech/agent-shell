...
export interface MockServerOptions {
  ...
  ttftDelay?: number;
  /** Shell cmd for the SSE tool_call (default: "echo hello from mock server"). */
  sseCmd?: string;
...
function mergeDefaults(opts: MockServerOptions): Required<MockServerOptions> {
  return {
    ...
    sseCmd: opts.sseCmd ?? "echo hello from mock server",
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for (let i = 0; i < opts.sseChunks; i++) {
        ...
        // NOTE: the API streams arguments as string fragments that the client CONCATENATES.
        // Each fragment must be a piece of ONE valid JSON document, so the joined result
        // is parseable. (Previously each fragment was independently JSON.stringify'd, which
        // produced invalid joined JSON like {"cmd":"echo "}"hello..." — breaking exec.)
        // The cmd is configurable via opts.sseCmd so tests can drive model-initiated exit
        // (e.g. cmd="credential drop").
        const fullArgs = `{"cmd":"${opts.sseCmd}"}`;
          ...
          toolCalls = [
            {
              ...
              function: {
                name: "shell",
                // fragment 1 of 2
                arguments: fullArgs.slice(0, mid),
              },
            },
          ];
        } else if (i === 2) {
          toolCalls = [
            {
              index: 0,
              function: {
                // fragment 2 of 2 (no id/name — continuation of index 0)
                arguments: fullArgs.slice(mid),
              },
            },
          ];
        }
...
if (import.meta.main) {
  ...
  if (args.includes("--self-test")) {
    ...
    // Optional timing knobs (defaults preserve pre-existing behavior):
    // --sse-delay: ms between SSE chunks (default 50)
    // --ttft-delay: ms before first SSE byte (default 0)
    const sseDelayIdx = args.indexOf("--sse-delay");
    const ttftDelayIdx = args.indexOf("--ttft-delay");
    const sseDelay = sseDelayIdx !== -1 ? parseInt(args[sseDelayIdx + 1]!, 10) : 50;
    const ttftDelay = ttftDelayIdx !== -1 ? parseInt(args[ttftDelayIdx + 1]!, 10) : 0;

    // --sse-cmd <cmd>: shell command embedded in the SSE tool_call. Accepts spaces
    // (consumed verbatim up to the next --flag or end of args).
    const sseCmdIdx = args.indexOf("--sse-cmd");
    let sseCmd: string | undefined;
    if (sseCmdIdx !== -1) {
      const rest = args.slice(sseCmdIdx + 1);
      const nextFlag = rest.findIndex((a) => a.startsWith("--"));
      sseCmd = (nextFlag === -1 ? rest : rest.slice(0, nextFlag)).join(" ");
    }

    const instance = await startMockServer({ port, responseType, sseChunks, sseDelay, ttftDelay, ...(sseCmd ? { sseCmd } : {}) });
