...
#
# agent.zsh sources these near the END of the file, once every definition is in
# place (function slots / banner ready) and just before the startup banner:
#   [[ -f ~/.agshrc ]] && source ~/.agshrc
#   [[ -f .agshrc ]] && source .agshrc
#
# Being after the idempotency guard, the files load once per shell (a re-source
# returns early). We test the EXACT loading semantics by running these same
# lines in a zsh subshell, avoiding side effects from the full agent.zsh.
