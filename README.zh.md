...
Agent Shell 是一个完全构建在 Zsh 上的终端原生 AI Agent。以最朴素的交互方式——
一行命令、一个灰色建议、一次自动执行——和节点链式上下文结构，
探索新型 Agent 的方向。

- **极简** — 单工具、零抽象、无状态机
- **低开销** — 无 tool definitions、无 LSP、无 checkpoint
- **可扩展** — 内核之外的一切,含自带标准库,都是普通插件,经 `.agshrc` 加载;内核不依赖其中任何部分
...
每个节点默认是 `.agsh/nodes/<id>/` 目录（可由 AGENT_NODES_PATH 重定位），包含三个文件：
...
内核(`agent.zsh` 与 `src/`)自足。内核之外的一切——包括自带的标准库 `agsh-stdlib`——都是插件,与任何普通插件地位无异:没有特殊的加载方式,内核也不依赖其中任何部分。

`.agshrc` 是加载一切非内核组件的唯一入口。它在 shell init 时被 source——先用户级 `~/.agshrc`,后项目级 `.agshrc`。要加载什么,写进去即可(中文界面有讲究:`i18n` 先于 `prompt`,`bootstrap` 最后):

```sh
# .agshrc
export AGENT_PRELOAD="${AGENT_PRELOAD:+$AGENT_PRELOAD }/abs/path/agsh-stdlib/stats.ts"   # 流统计
source /abs/path/agsh-stdlib/rprompt.zsh                # RPROMPT 状态图标
source /abs/path/agsh-stdlib/cleanup.zsh                # 回收陈旧临时目录
source /abs/path/agsh-stdlib/i18n.zsh                   # 语言套件(自动加载 lang/、help.zsh、kernel-slots.zsh)
source /abs/path/agsh-stdlib/prompt.zsh                 # prompt 节点助手(自足;i18n 负责翻译)
source /abs/path/agsh-stdlib/bootstrap.zsh              # 首次运行建 root
```

`AGENT_PRELOAD` 是**一个**空格分隔的字符串,内核把每一项变成一个 `--preload`。所以插件要**追加**自己的路径,而不是整个赋值——这样插件才能各自独立地加进来、摘出去:

```sh
export AGENT_PRELOAD="${AGENT_PRELOAD:+$AGENT_PRELOAD }/abs/path/plugin.ts"
```

两种写错的后果:`export AGENT_PRELOAD+=...` 在 zsh 里是**语法错误**;而直接 `export AGENT_PRELOAD='...'` 是**整值覆盖**,第二个插件会把第一个**悄悄抹掉**。顺序即加载顺序;两个插件包同一个东西时(比如都包 `agsh.api.stream`),后装者在外层。

`i18n.zsh` 是套件唯一入口:它自行加载 `i18n-core.zsh`、`help.zsh`、`kernel-slots.zsh` 与语言包 `lang/<id>.zsh`——不要直接 source 这些文件。`lang/en.zsh` 承载全部英文文本(含内核表面),`lang/zh.zsh` 在其上覆盖。用 `agent-lang list` / `agent-lang set zh` 选择语言;默认跟随 `LANG`,`STDLIB_I18N_STRICT=1` 时缺键告警但不改变文本。

上面几行加载的是自带的标准库——它也是一个插件,与其它任何插件无异。什么都不加载,内核照常运行:图标、统计、`prompt` 助手与中文界面都是标准库自己的特性,不是内核的特性,谈不上缺失。没有 `prompt.zsh` 时,建节点就是 `mkdir` 加 `context`、`parent` 两个文件(节点链协议自带的做法)。

**协议文本**只有一处权威:`src/protocol.ts`(纯英文,永不本地化)。插件可经 `AGENT_PRELOAD` 覆盖 `agsh.init.protocol`;显式 `agsh init` 与首次运行的 bootstrap 都会转发 preload。既有的 `root/context` 是创建时的快照——内核不校验、不同步、不重播种。

写自己的插件:插入点在源码里——读 `agent.zsh` 与 `src/`;`agsh-stdlib/` 是一个现成的例子。
...
> `prompt` 由自带标准库 `agsh-stdlib` 提供(经 `.agshrc` 加载),内核不提供该命令——见[插件](#插件)。
...
> 图标与实时统计由自带标准库渲染(`agsh-stdlib/rprompt.zsh` + `stats.ts`),
> 经 `.agshrc` 加载,见[插件](#插件)。不加载就没有图标与统计;内核本身不变。
...
| `AGENT_BASE_URL` | （必填） | 兼容 chat-completions 的 API 端点（无供应商默认值） |
| `AGENT_MODEL` | （必填） | 模型名称 |
| `AGENT_EXEC_DELAY` | `2` | 建议自动执行延迟（秒） |
| `AGENT_OUTPUT_MAX_LENGTH` | `10000` | 工具输出截断长度（字符） |
| `AGENT_DEBUG` | `false` | 调试日志 |
| `AGENT_EXEC_TIMEOUT` | `0` | 命令执行超时（秒，0=禁用） |
| `AGENT_REASONING_EFFORT` | `high` | 推理深度 |
| `AGENT_API_TTFT_TIMEOUT` | `120` | 流式首个 token 超时（秒，0=禁用） |
| `AGENT_POLL_INTERVAL` | `0.05` | shell 轮询间隔（秒） |
| `AGENT_NODES_PATH` | `.agsh/nodes` | 节点树根目录 |

优先级（高到低）：`.agshrc` > `~/.agshrc` > 环境变量 > `.env` > 内置默认值
