...
const DUMP_DIR = "__tests__/tmp-test-context-dump";
describe("context", () => {
  beforeEach(async () => {
  ...
  describe("readHistory (via buildContext)", () => {
    ...
    it("single valid JSON array line → returns array contents in context", async () => {
      ...
      // (a kernel-injected <recover> user message is also present, since history ends with a text-only assistant)
      const realUsers = messages.filter(
        (m) => m.role === "user" && !String(m.content).startsWith("<recover>"),
      );
    ...
    it("malformed JSON line mixed with valid lines → skips malformed, returns valid content", async () => {
      ...
      // Only the valid JSONL lines are parsed (plus the kernel-injected <recover> user message)
      const realUsers = messages.filter(
        (m) => m.role === "user" && !String(m.content).startsWith("<recover>"),
      );
      expect(realUsers).toHaveLength(1);
  ...
  describe("saveDelta", () => {
    it("context file does not exist → returns {saved: 0, total: 0}", async () => {
      ...
      const result = saveDelta("agent", TEST_DIR, ctxPath, 0, DUMP_DIR);
    ...
    it("fromIndex < 0 → returns {saved: 0, total: context.length}", async () => {
      ...
      const result = saveDelta("agent", TEST_DIR, ctxPath, -1, DUMP_DIR);
    ...
    it("fromIndex >= context.length → returns {saved: 0, total: context.length}", async () => {
      ...
      const result = saveDelta("agent", TEST_DIR, ctxPath, 10, DUMP_DIR);
    ...
    it("fromIndex in middle of array → saves delta, returns correct saved count", async () => {
      ...
      const result = saveDelta("agent", TEST_DIR, ctxPath, 2, DUMP_DIR);
    ...
    it("verifies history file is written with correct JSONL delta", async () => {
      ...
      saveDelta("agent", TEST_DIR, ctxPath, 0, DUMP_DIR);
