...
describe("callApiStream", () => {
  ...
  function setStreamEnv(baseURL: string) {
    ...
    // callApiStream's default `config` parameter runs loadConfig(), which
    // requires AGENT_MODEL even though the model is passed explicitly.
    process.env.AGENT_MODEL = "test-model";
  }
  ...
  it("yields error when response body is null", async () => {
    ...
    try {
      globalThis.fetch = (() =>
        Promise.resolve(new Response(undefined, { status: 200 }))) as unknown as typeof fetch;
  ...
  it("handles empty stream yielding only done event with minimal content", async () => {
    ...
    // Empty stream: no usage chunk ever arrived, so done carries no usage.
    // (usage is API-raw passthrough, never synthesized by core).
    const contentEvents = filterByType(events, "content");
  ...
  it("includes usage object in done event from final chunk", async () => {
    ...
    // token events are no longer emitted by core (stats moved to stdlib plugin)
